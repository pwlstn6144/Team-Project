...
<%
session.setAttribute("bUrl", "workerList"); 
	...
	function deleteWorker(){
		...
		for(var i = 0; i < worker.length; i++){
			if(worker[i].checked == true){
				checkTrue.push(worker[i].value);
			}
		}
		var check = confirm(checkTrue.length+"명을 삭제하시겠습니까?");
		...
		if(check == true){
			$.ajax({
				url:'./deleteWorker',
				type: 'POST',
				data : {wk_Id:checkTrue},
				dataType:'json',
				success : function(json){
					var results = eval(json);
					if(results.result=="ok"){
						alert("삭제되었습니다.");		
						document.location.href="workerList?page="+page.value;
					}
					else{
						alert(results.desc);
					}
...
function frm_submit(){
	...
	if(bsearch == "workerName"){
		searchValue.value = "workerName";
	}
	else if(bsearch == "workerPhone"){

		searchValue.value = "workerPhone";
	}
	else if(bsearch == "workerId"){

		searchValue.value = "workerId";
	}
...
<script>
	$(document).ready(function(){
		$('.all').click(function(){
			$('.pice').prop('checked', this.checked);
		});
	});
</script>
...
<div class="container">
	...
	<table class="table">
		...
		<tr>
			<th scope="col"><input type="checkbox" id="allCheck" name="allCheck" class="all" onclick="all()">전체</th>
			<th scope="col">이름</th>
		...
		<tr>
			<td><input type="checkbox" name="worker" class="pice" value="${dto.wk_Id }"></td>	
			<td>${dto.wk_Name}</td>
			<td>${dto.wk_Phone}</td>
			<td><a href="work_view?wk_Id=${dto.wk_Id}&page=${page.curPage}">${dto.wk_Id}</a></td>
			<c:if test="${dto.wk_Mode == 0}">
				<td>배송 대기 중</td>
			</c:if>
			<c:if test="${dto.wk_Mode == 1}">
				<td>배송 중</td>
			</c:if>
			<c:if test="${dto.wk_Mode == 2}">
				<td>휴식 중</td>
			</c:if>
			<c:if test="${dto.wk_Mode == 3}">
				<td>퇴근 중</td>
			</c:if>
			<td>${dto.wk_Penalty}</td>
		</tr>
		</c:forEach>
		<tbody>	
		<tr>
				...
				<form action="search" name="frm_search"> 
					<c:if test="${info eq null }">
						<select class="custom-select" id="bSearch" name="bSearch" style="width:150px"> 
							<option value="workerName" >이름 
							<option value="workerPhone">전화번호
							<option value="workerId">아이디
						</select>
					</c:if>
					<c:if test="${info eq ''}">
						<select class="custom-select" id="bSearch" name="bSearch" style="width:150px"> 
		...
		<tr>
			...
			<nav aria-label="..." class="nav justify-content-center">
					...
					<c:choose>
						<c:when test="${(page.curPage -1) < 1}">
							<li class="page-item disabled" >
								<a class="page-link" href="workerList?page=1&info=${info}&search=${search}" tabindex="-1">처음</a>	
							</li>
					</c:when>
					<c:otherwise>
						<li class="page-item">
							<a class="page-link" href="workerList?page=1&info=${info}&search=${search}">처음</a>				
						</li>
					</c:otherwise>
				</c:choose>
					...
					<c:choose>
						<c:when test="${page.curPage == fEach}">
							<li class="page-item active">
								<a class="page-link" href="workerList?page=${fEach}&info=${info}&search=${search}">${fEach}<span class="sr-only">(current)</span></a> 
							</li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="workerList?page=${fEach}&info=${info}&search=${search}">${fEach}</a></li> 				
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="workerList?page=${page.totalPage}&info=${info}&search=${search}">끝</a></li>				
					</c:otherwise>
				</c:choose>
				</ul>
			</nav>
