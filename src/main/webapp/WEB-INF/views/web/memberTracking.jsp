...
<div class="container">
	...
	<table class="table">
		...
		<tr>
			<th scope="row">${dto.cst_orderid}</th>
			<td>${dto.cst_Name}</td>
			<td>${dto.arrive_Name}</td>
			<td>
				<a href="trackingView?cst_orderid=${dto.cst_orderid}">${dto.orderRequestNumber}</a>
		...
		<tr>
			...
			<form action="search" name="frm_search"> 
				...
				<c:if test="${info eq 'orderInvoiceNumber' }">
					<select class="custom-select" id="bSearch" name="bSearch" style="width:150px">
						<option value="orderRequestNumber">주문 번호 
						<option value="arriveName">받는 사람
						<option value="cstName" >보내는 사람 				
			...
			<ul class="pagination" id="nav">	
				<c:choose>
					<c:when test="${(page.curPage -1) < 1}">
						<li class="page-item disabled" >
							<a class="page-link" href="tracking?page=1&info=${info}&search=${search}" tabindex="-1">처음</a>
						</li>
					</c:when>
					<c:otherwise>
						<li class="page-item">
							<a class="page-link" href="tracking?page=1&info=${info}&search=${search}">처음</a>	
						</li>			
					</c:otherwise>
				</c:choose>
					...
					<c:choose>
						<c:when test="${page.curPage == fEach}">
							<li class="page-item active">
								<a class="page-link" href="tracking?page=${fEach}&info=${info}&search=${search}" >${fEach}<span class="sr-only">(current)</span></a>
							</li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="tracking?page=${fEach}&info=${info}&search=${search}">${fEach}</a></li> 				
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="tracking?page=${page.totalPage}&info=${info}&search=${search}">끝</a></li>			
					</c:otherwise>
				</c:choose>
			</ul>
			</nav>
