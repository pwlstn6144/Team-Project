...
<%
session.setAttribute("bUrl", "wk_notice"); 
		...
		<tr>
			<td colspan="5">
					...
					<c:choose>
						<c:when test="${(page.curPage -1) < 1}">
							<li class="page-item disabled" >
								<a class="page-link" href="wk_notice?page=1&info=${info}&search=${search}" tabindex="-1">처음</a>	
							</li>
					</c:when>
					<c:otherwise>
						<li class="page-item">
							<a class="page-link" href="wk_notice?page=1&info=${info}&search=${search}">처음</a>				
						</li>
					</c:otherwise>
				</c:choose>
					...
					<c:choose>
						<c:when test="${page.curPage == fEach}">
							<li class="page-item active">
								<a class="page-link" href="wk_notice?page=${fEach}&info=${info}&search=${search}">${fEach}<span class="sr-only">(current)</span></a> 
							</li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="wk_notice?page=${fEach}&info=${info}&search=${search}">${fEach}</a></li> 				
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="wk_notice?page=${page.totalPage}&info=${info}&search=${search}">끝</a></li>				
					</c:otherwise>
				</c:choose>
				</ul>
				</nav>
